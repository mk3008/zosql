...
export class WorkspacePanelShadowComponent extends ShadowComponentBase {
  ...
  handleCteTreeItemClick(cteName) {
    console.log(`[WorkspacePanelShadow] handleCteTreeItemClick called for: ${cteName}`);
    ...
    console.log(`[WorkspacePanelShadow] CTE data for ${cteName}:`, cteData);
    console.log(`[WorkspacePanelShadow] CTE ${cteName} dependencies:`, cteData.dependencies);
    ...
    console.log(`[WorkspacePanelShadow] CTE query for ${cteName}:`, {
    ...
    // contentはnullを渡して、WorkspaceServiceで依存関係を含むSQLを生成してもらう
    this.openWorkspaceFile(`${cteName}.cte`, 'cte', null);
  }
  ...
  async openWorkspaceFile(fileName, type, content = null) {
    try {
      console.log(`[WorkspacePanelShadow] openWorkspaceFile called:`, {fileName, type, hasContent: !!content});
      
      // コンテンツの取得
      let fileContent = content;
      console.log(`[WorkspacePanelShadow] Initial fileContent:`, fileContent ? `${fileContent.length} chars` : 'null');
      
      if (!fileContent) {
        console.log(`[WorkspacePanelShadow] No initial content, trying to fetch...`);
        ...
        if (window.fileModelManager) {
          ...
          if (existingModel) {
            ...
            console.log(`[WorkspacePanelShadow] Got content from fileModelManager:`, fileContent ? `${fileContent.length} chars` : 'null');
          }
        }
        
        // CTEデータから直接取得を試行（コメントアウト - WorkspaceServiceで依存関係を含むSQLを生成する）
        // if (!fileContent && type === 'cte' && this.cteDependencyData && this.cteDependencyData.privateCtes) {
        //   const cteName = fileName.replace('.cte', '');
        //   const cteData = this.cteDependencyData.privateCtes[cteName];
        //   console.log(`[WorkspacePanelShadow] Processing CTE: ${cteName}`);
        //   console.log(`[WorkspacePanelShadow] CTE data:`, cteData);
        //   
        //   if (cteData && cteData.query) {
        //     // CTEデータからはそのまま取得（既にextractCTEQuery処理済み）
        //     fileContent = cteData.query;
        //     console.log(`[WorkspacePanelShadow] CTE ${cteName} content length:`, fileContent.length);
        //     console.log(`[WorkspacePanelShadow] CTE ${cteName} content preview:`, fileContent.substring(0, 100));
        //   } else {
        //     console.warn(`[WorkspacePanelShadow] No query data found for CTE: ${cteName}`);
        //   }
        // }
        ...
        if (!fileContent) {
          try {
            ...
            if (response.ok) {
              ...
              console.log(`[WorkspacePanelShadow] API response for ${type}/${requestFileName}:`, result);
